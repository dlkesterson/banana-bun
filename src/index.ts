...
import { performStartupHealthCheck } from './utils/service-health';
...
async function processTaskFile(filePath: string): Promise<void> {
    ...
    while (
        ...
        const timestamp = new Date()
            ...
            .replace(/[-:T.]/g, "")
            .slice(0, 15);
        uniqueName = `${nameNoExt}_${timestamp}${counter > 1 ? "_" + counter : ""
            }${ext}`;
    }
...
async function main() {
    try {
        ...
        // Perform startup health check for external services
        ...
        if (!healthCheck.success) {
            ...
            console.log("   Unhealthy services:", healthCheck.unhealthyServices.join(", "));
            console.log("");
            console.log("💡 To start services automatically, run:");
            console.log("   bun run dev:services");
            console.log("   or manually: .\\start-services-windows.ps1");
            console.log("");
            console.log("⚠️ Continuing without some services - functionality may be limited");
        } else {
