...
/**
 * Atlas CLI tool for generating content recommendations
 * 
 * Usage:
 *   bun run src/cli/atlas-recommend.ts --media 123
 *   bun run src/cli/atlas-recommend.ts --user alice --top 10
 *   bun run src/cli/atlas-recommend.ts --user alice --media 123 --type hybrid
 */

import { parseArgs } from 'util';
import { initDatabase, getDatabase } from '../db';
import { logger } from '../utils/logger';
import { createMediaRecommendTask, recordUserInteraction } from '../executors/recommend';
...
function printUsage() {
    console.log(`
Atlas Content Recommendation Tool

Usage: bun run src/cli/atlas-recommend.ts [options]
...
Examples:
  ...
  bun run src/cli/atlas-recommend.ts --user alice --media 123 --record play

  # Run directly without creating a task
  bun run src/cli/atlas-recommend.ts --media 123 --direct
...
function parseCliArgs(): CliOptions {
    ...
    if (values.type) {
        if (!['similar', 'user_based', 'hybrid'].includes(values.type)) {
            throw new Error(`Invalid recommendation type: ${values.type}. Must be one of: similar, user_based, hybrid`);
        }
    ...
    if (values.top) {
        ...
        if (isNaN(top) || top < 1 || top > 50) {
            throw new Error(`Invalid top value: ${values.top}. Must be between 1 and 50`);
        }
        options.top = top;
    }

    if (values.record) {
        const validActions = ['play', 'like', 'share', 'complete', 'skip', 'search_click'];
        if (!validActions.includes(values.record)) {
            throw new Error(`Invalid action: ${values.record}. Must be one of: ${validActions.join(', ')}`);
        }
...
async function validateInputs(options: CliOptions): Promise<{ valid: boolean; error?: string }> {
    ...
    if (!options.type) {
        if (options.mediaId && options.userId) {
            ...
            return { valid: false, error: 'Either --media or --user must be specified' };
        }
    }

    // Validate required parameters for each type
    if (options.type === 'similar' && !options.mediaId) {
        return { valid: false, error: 'Media ID is required for similar recommendations' };
    }

    if ((options.type === 'user_based' || options.type === 'hybrid') && !options.userId) {
        return { valid: false, error: 'User ID is required for user-based and hybrid recommendations' };
    }
...
async function recordInteraction(options: CliOptions): Promise<void> {
    ...
    console.log(`📝 Recording ${options.record} interaction for user ${options.userId} on media ${options.mediaId}...`);
...
async function runDirectRecommendations(options: CliOptions): Promise<void> {
    ...
    switch (options.type) {
        ...
        case 'user_based':
            result = await recommenderService.getUserRecommendations(options.userId!, {
                topK: options.top || 10,
                includeReason: true
            });
            break;
        ...
        case 'hybrid':
            ...
            console.log('⚠️  Hybrid recommendations require task creation. Use --direct=false or omit --direct flag.');
            return;
    ...
    if (!result.recommendations || result.recommendations.length === 0) {
        ...
        result.recommendations.forEach((rec, index) => {
            console.log(`${index + 1}. ${rec.metadata?.filename || `Media ID ${rec.media_id}`}`);
            console.log(`   Score: ${Math.round(rec.score * 100)}%`);
            if (rec.reason) {
                console.log(`   Reason: ${rec.reason}`);
            }
            if (rec.metadata?.duration) {
                console.log(`   Duration: ${Math.round(rec.metadata.duration / 60)}m ${Math.round(rec.metadata.duration % 60)}s`);
            }
...
async function main() {
    try {
        ...
        console.log('🎯 Atlas Content Recommendation Tool');
        console.log('====================================\n');
