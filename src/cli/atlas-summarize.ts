...
/**
 * Atlas CLI tool for generating content summaries
 * 
 * Usage:
 *   bun run src/cli/atlas-summarize.ts --media 123
 *   bun run src/cli/atlas-summarize.ts --media 123 --style bullet --model gpt-4
 *   bun run src/cli/atlas-summarize.ts --media 123 --force
 */
...
function printUsage() {
    console.log(`
Atlas Content Summarization Tool

Usage: bun run src/cli/atlas-summarize.ts [options]
...
Examples:
  bun run src/cli/atlas-summarize.ts --media 123
  bun run src/cli/atlas-summarize.ts --media 123 --style paragraph
  bun run src/cli/atlas-summarize.ts --media 123 --model gpt-4 --force
  bun run src/cli/atlas-summarize.ts --media 123 --direct
...
function parseCliArgs(): CliOptions {
    ...
    if (values.style) {
        if (!['bullet', 'paragraph', 'key_points'].includes(values.style)) {
            throw new Error(`Invalid style: ${values.style}. Must be one of: bullet, paragraph, key_points`);
        }
...
async function validateMediaExists(mediaId: number): Promise<{ exists: boolean; hasTranscript: boolean; filePath?: string }> {
    ...
    const mediaRow = db.prepare(`
        ...
        WHERE mm.id = ?
    `).get(mediaId) as { file_path: string; transcript_id?: number; transcript_text?: string } | undefined;
...
async function runDirectSummarization(options: CliOptions): Promise<void> {
    ...
    const result = await summarizerService.generateSummaryForMedia(options.mediaId, {
...
async function createSummarizationTask(options: CliOptions): Promise<void> {
    ...
    console.log(`📋 Creating summarization task for media ID ${options.mediaId}...`);
...
async function main() {
    try {
        ...
        console.log('🎬 Atlas Content Summarization Tool');
        console.log('===================================\n');
        ...
        if (!validation.hasTranscript) {
            ...
            console.log('💡 Tip: Run transcription first with: bun run src/cli/smart-transcribe.ts');
            process.exit(1);
        }
